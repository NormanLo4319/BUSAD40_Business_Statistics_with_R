...
Given the following situations, which of the probability distribution woudl be the most appropriate to apply?
...
1. As an quality control inspector, your job is to ensure the product quality meeting the company standard. Inspectors usually check the product from the production and count the number of inspected items until they find a defect. 
...
4. In the World Cup, an average of 2.5 goals are scored each game. Modeling this situation we can use a distribution to approaximate the probability that k goals are scored in a game.
...
1. Suppose you are one of the adminstration staff who handle the applications. Based on this inforamtion, what is your probability that you randomly pick an application from a female applicant? $P(Gender=Female)$
...
6. What is the proability a randomly picked application is applying either department B, D, or F? $P(Department=B \cup Department=D \cup Department=F)$
...
8. What is the probability a randomly selected application is admitted by UCB given that the applicant is a female? $P(Adimitted=Yes | Gender=Female)$
...
Suppose you are running a small local busniess.  You have a shipment just arrived from your supplier and below is the table of the summary about the quality of the shipment.
...
Given the summary table, answer the following probability quesitons: (Show Work!!)
...
2. What is the probability for your custoemr randomly pick an item that is a bag and defect?
...
3. What length of time marks the shortest 75% of all shippment?

Ans:

4. What is the upper bound and lower bound lenght of time for a shippment arrive within 2 standard deviation from the mean?
